...
#################################################################### Start ##########################################################################################################################################
## load(file = "RNA-WT-AdultCM_TSS.RData")                
mySample_g   <- "RNA-WT-AdultCM";
myTitle_g    <- "TSSs of all genes";
myCenter_g   <- "TSS";
AllResults_g <- paste("Z_Figures",  mySample_g,  "TSS",  sep = "/")
...
myNTR_withEquation_2 <- function( xAxis2,  yAxis2,   path2,  fileName2, titel2, xlab2, ylab2 ,yMin2=0, yMax2=0.7,   xMin2=0, xMax2=0.7 ) { 
  ...
  regressionA <- lm(yAxis2 ~ xAxis2)
  FigureTemp1 <- ggplot(data = dataFrame_NTR_point, aes(x = xAxis, y = yAxis)) + 
    geom_point(size=0.2, alpha=0.2, color="red") + geom_smooth(method = "lm", se=FALSE, color="blue", formula = y ~ x) +  
    geom_text( aes(x = 0.7, y = 0.4,  label = MyLmEquation_1( regressionA )), parse = TRUE,  colour="cyan4", size=2.5 ) +
    xlab(xlab2) +  ylab(ylab2) + ggtitle(titel2) + MyTheme_1( hjust1=1, vjust1=1,    textSize=12 )
  MySaveGgplot2_1(ggplot2Figure1=FigureTemp1,  path1=path2, fileName1=paste(fileName2, "-scatter-equation", sep="",  collapse=NULL),  height1=4, width1=4)
}
